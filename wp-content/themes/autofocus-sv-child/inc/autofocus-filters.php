...
function af_layout_class($class = '') {
	...
	// SVEdit: The first if statement added for SV customization, Jan 2012, the rest is default AF
	// we want the Catalogue page to use the AF grid format
	if ( is_category( 'catalogue' ) ) {
		$af_classes[] = 'af-layout af-grid';
	} elseif ( $archive_layout == 'default' && ( is_archive() || is_search() ) ) {
	...
	if ( is_category( 'catalogue' ) ) {
		$af_classes[] = 'af-grid';
	} elseif ( ( is_home() && $home_layout == 'default' ) )
		$af_classes[] = 'af-default';
	elseif ( ( is_home() && $home_layout == 'grid' ) ) 
		$af_classes[] = 'af-grid';

	// SVEdit, top line, doesn't make a difference
	if (  (is_category( 'catalogue' )  )) {
		$af_classes[] = 'af-grid';
	} elseif ( $home_layout == 'default' && $archive_layout == 'images' && ( is_archive() || is_search() ) ) 
		$af_classes[] = 'af-default';
