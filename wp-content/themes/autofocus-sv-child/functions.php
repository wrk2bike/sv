 ...
 *
 *
 * In the case of the SV Autofocus child theme, we have renamed the original Functions.php 
 * so it won't be used, copied that file here, and modified the include paths for the
 * /inc/ files that have been modified - see line 83 and 88.  The rest is copied from Autofocus.
 * 
 */
...
// SVEdit: Our customized ones
// require_once(TEMPLATEPATH . '/inc/autofocus-images.php');
require_once(STYLESHEETPATH . '/inc/autofocus-images.php');

//	Load AutoFocus WP Filters
// SVEdit: Our customized ones
// require_once(TEMPLATEPATH . '/inc/autofocus-filters.php');
require_once(STYLESHEETPATH . '/inc/autofocus-filters.php');
...
function af_enqueue_styles() {
	...
	if ( !is_admin() ) { // Is this necessary?
		...
		//	Add iPhone Styles
		if ( of_get_option($shortname . '_iphone' ) == TRUE )
			wp_enqueue_style('iphone', TEMPLATE_DIR . '/css/style.iphone.css', '', '2.0', 'screen' );

		//	Add Fancybox script to single pages if the option is turned on
		if ( is_single() && ( of_get_option($shortname . '_fancybox' ) == TRUE ) )
			wp_enqueue_style('fancybox', TEMPLATE_DIR . '/js/fancybox/jquery.fancybox-1.3.1.css', '', '1.3.1' );
	}
}
add_action('wp_print_styles', 'af_enqueue_styles');
...
function af_enqueue_scripts() {
	...
	if ( !is_admin() ) { // Is this necessary?
		...
		//	Add jQueryUI for Drag & Drop image cropping
		if ( is_user_logged_in() && ( is_home() || is_archive() || is_search() ) )
			wp_enqueue_script('jqueryui', TEMPLATE_DIR . '/js/jquery-ui-1.8.13.custom.min.js', array('jquery'), '1.8.13' );
		...
		if ( of_get_option($shortname . '_sliding_sticky_area') == TRUE || is_single() )
			wp_enqueue_script('cycle', TEMPLATE_DIR . '/js/jquery.cycle.all.min.js', array('jquery', 'easing'), '1.2' );

		//	Add Fancybox script to single pages if the option is turned on
		if ( is_single() && ( of_get_option($shortname . '_fancybox' ) == TRUE ) ) {
			wp_enqueue_script('fancybox', TEMPLATE_DIR . '/js/fancybox/jquery.fancybox-1.3.1.pack.js', array('jquery', 'easing'), '1.3.1' );
		}
...
//SVEdit: show only one category on the home page
add_filter( 'pre_get_posts', 'my_get_posts' );

function my_get_posts( $query ) {
  if ( ( is_home() && false == $query->query_vars['suppress_filters'] ) )
    $query->set( 'category_name', 'newest' );
  return $query;
}
