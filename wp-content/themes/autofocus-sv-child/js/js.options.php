...
<?php }
	...
	// SVEdit - this one does the title, date & description on mouseover
	// Note: the description is not meant by the layout to appear in grid view due to potential length, I think
	if ( (is_home() && $home_layout == 'default') || is_category( 'catalogue' ) || ( $archive_layout == 'images' && $home_layout == 'default' && ( is_archive() || is_search() ) ) ) {
	...
	//
	// SVEdit: Attach the mouseover stuff to the Catalogue page becuase we expect it in grid format
	// This section adds mouseover actions showing the title image and date, but not the description.
	// This is because the description is potentially too large for the grid layout I think, and the left padding is off.  
	// Use the description in the section above if you want it, and keep this defaulted here to the original code.
	//
	// New:
	// if ( ( is_home() && $home_layout == 'grid') || is_category( 'catalogue' )  || ( $archive_layout == 'images' && $home_layout == 'grid' && ( is_archive() || is_search() ) ) ) {
